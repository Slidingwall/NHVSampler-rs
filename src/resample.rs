...
pub fn resample(args: Arguments) -> Result<()> {
    ...
    let resonance = args.flags.get("Hr").and_then(|x| *x).unwrap_or(0.0);
    ...
    if let Some(&a) = args.flags.get("A").and_then(|x| x.as_ref()) {
        ...
        if n > 1 {
            ...
            for i in 1..n - 1 {
                g[i] = (a * ((pitch_render[i + 1] - pitch_render[i - 1]) * 0.5) * 2.321928094887362).exp2();
            }
    ...
    let drive = args.flags.get("HD").and_then(|x| *x).map_or(0.0, |v| v.clamp(0.0, 100.0) / 100.0);
    let unease = args.flags.get("Hp").and_then(|x| *x).map_or(0.0, |v| v.clamp(0.0, 100.0) / 100.0);
